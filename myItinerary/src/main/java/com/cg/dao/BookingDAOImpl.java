...
public class BookingDAOImpl implements BookingDAO {
	...
	private static final String SQL_CANCEL_RESORT_BY_RESORT_ID = "UPDATE resort_reservation SET status=:sts WHERE r_reservation_number=:rid";
	...
	private static final String SQL_CANCEL_DINING_BY_DINING_ID = "UPDATE dining_reservation SET status=:sts WHERE d_reservation_number=:did";
	...
	public Resort cancelBookResort(long resortBookingId) {
		...
		try {
			MapSqlParameterSource params = new MapSqlParameterSource().addValue("sts", status).addValue("rid",
					resortBookingId);

			jT.update(SQL_CANCEL_RESORT_BY_RESORT_ID, params);
			log.info("BookingDAOImpl:Resort Booking Cancelled ");
		} catch (Exception e) {
			log.warn("BookingDAOImpl:Cancelling Resort failed.Resort booking id not found");
			return null;
		}
		return getResortByResortBookingID(resortBookingId);
	}
	...
	public Dining cancelBookedDining(long diningBookingId) {
		...
		String status = "cancelled";
		try {
			MapSqlParameterSource params = new MapSqlParameterSource().addValue("sts", status).addValue("did",
					diningBookingId);

			jT.update(SQL_CANCEL_DINING_BY_DINING_ID, params);
		} catch (Exception e) {
			log.warn("BookingDAOImpl:Resort Not Cancelled.");
			return null;
		}
		return getDiningByDiningBookingID(diningBookingId);
	}
